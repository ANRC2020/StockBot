...
class stock:
    ...
    prev_momentum_EMA = 0
    momentum_counter = 2
    momentumlong_EMA_log = []
    curr_momentum_EMA_long = 0
    prev_momentum_EMA_long = 0
    momentum_counter_long = 2
    ...
    def __init__(self,first_line):
        ...
        self.momentumlong_EMA_log = []
        self.curr_momentum_EMA_long = 0
        self.prev_momentum_EMA_long = 0
        self.momentum_counter_long = 2
...
def percent_change_update(stock, x, IL):
    ...
    # #Check to see if the stoploss was triggered
    # if(stock.stoplosslimit > stock.low[-1]):
    #     stoplosssell(stock)

    # #Update Stoplosslimit to new previous close
    # if(stock.bought == True and stock.sold == False):
    ...
    #Update Long Term Momentum EMA (26 periods)
    if(len(stock.momentumlong) >= 1):
        stock.prev_momentum_EMA_long = stock.curr_momentum_EMA_long
        stock.curr_momentum_EMA_long = stock.momentumlong[-1]*(2/(stock.momentum_counter_long+1)) + stock.curr_momentum_EMA_long*(1 - (2/(stock.momentum_counter_long + 1)))
        stock.momentumlong_EMA_log.append(stock.curr_momentum_EMA_long)
        stock.momentum_counter_long += 1
    else:
        stock.momentumlong_EMA_log.append(0)
...
def sell(stock):
    ...
    # if(stock.capital != stock.section_val):
        # input(f"stock.capital = {stock.capital} and stock.section_val = {stock.section_val}")
...
if __name__ == "__main__":
    ...
    rs.login(username='siddiquiabbas22@gmail.com',
         password='Siddiqui#123!!ANRC2001',
         expiresIn=86400,
         by_sms=True)
    ...
    for i in range(len(symbol)):
        ...
        data = rs.stocks.get_stock_historicals(symbol[i], interval=increment, span=timeframe)

        title = f"TestDataSet{i}.csv"
        filelist.append(title)

        with open(title, 'w', newline = '') as csvfile:
            fieldnames = ['Open', "High", "Low", "Close", "Volume", 'Date']
            ...
            for j in range(len(data)):
                ...
                writer.writerow({'Open': data[j]['open_price'], 'High': data[j]['high_price'], 'Low': data[j]['low_price'], 'Close':data[j]['close_price'], 'Volume':data[j]['volume'], 'Date':data[j]['begins_at']})
            csvfile.close()
                ...
                for i in range(stocks):
                    ...
                    if(IP == 0):
                        ...
                        if(stocks_list[i].hard_sell == False):
                            ...
                            # if(stocks_list[i].momentum_EMA_log[-1] < 0 and stocks_list[i].momentum_EMA_log[-1] < stocks_list[i].momentum_EMA_log[-2]):
                            ...
                            # elif(stocks_list[i].momentum_EMA_log[-1] > 0 and stocks_list[i].momentum_EMA_log[-1] > stocks_list[i].momentum_EMA_log[-2]):
                                ...
                                # if(stocks_list[i].PSARMatrix[0][7] == 1 and stocks_list[i].PSARMatrixlong[0][7] == 1):
                                    if(stocks_list[i].bought == False and stocks_list[i].sold == True):
                                        if(stocks_list[i].EMA_Slope > 0):
                                            # if(stocks_list[i].momentum_EMA_log[-1] > 0 and stocks_list[i].momentum_EMA_log[-1] > stocks_list[i].momentumlong_EMA_log[-1] and stocks_list[i].momentum_EMA_log[-1] > stocks_list[i].momentum_EMA_log[-2]):
                                            # if(stocks_list[i].momentum_EMA_log[-1] > 0 and stocks_list[i].momentum_EMA_log[-1] > stocks_list[i].momentum_EMA_log[-2]):
    ...
    for i in range(stocks):
        ...
        print(f"Number of winning trades: {stocks_list[i].num_wins}")
        print(f"Number of losing trades: {stocks_list[i].num_losses}")
    ...
    if(y == 'y' or y == 'Y'):
        ...
        if(y != ''):
            for stock in stocks_list:
                ...
                axis[2].plot(stock.t, stock.RSI, 'm', stock.t, [70]*len(stock.t), 'k', stock.t, [30]*len(stock.t), 'k', stock.t, [50]*len(stock.t), 'k', stock.t, stock.RSILong, 'b')
                axis[2].set_title(stock.Ticker + ' Short RSI and Long RSI')

                axis[3].plot(stock.t, stock.momentum_EMA_log, 'r', stock.t, stock.momentumlong_EMA_log, 'b')
                axis[3].set_title(stock.Ticker + ' Momentum EMA')
                plt.show()
        # for stock in stocks_list:
        #     x = input()
        #     if(x != ''):
        ...
        #         axis[0].set_title(stock.Ticker + ' With RSI')

        #         axis[1].plot(stock.t, stock.RSI, 'm', stock.t, [70]*len(stock.t), 'b', stock.t, [30]*len(stock.t), 'b')
        #         plt.show()

        # plt.plot(stock.t, stock.close, 'k', stock.t, stock.buyindicator, 'og', stock.t, stock.soldindicator, 'or', stock.t, stock.histLongPSAR, 'xc', stock.t, stock.histShortPSAR, 'xm')
        # plt.title(stock.Ticker + 'With RSI')
        # plt.show()

        # plt.plot(stock.t, stock.RSI, 'm')
        # plt.show()
