...
def moving_avg_update(stock):
    ...
    if(len(stock.SMA_13arr) >= 2):
        stock.SMA_13arrSlope.append(float(stock.SMA_13arr[-1] - stock.SMA_13arr[-2])/float(timestep))
        stock.SMA_8arrSlope.append(float(stock.SMA_8arr[-1] - stock.SMA_8arr[-2])/float(timestep))
        stock.SMA_5arrSlope.append(float(stock.SMA_5arr[-1] - stock.SMA_5arr[-2])/float(timestep))
        print(f"\nStock Symbol = {stock.Ticker}")
        print(f"SMA_13arrSlope.[latest] = {stock.SMA_13arrSlope[-1]}")
        print(f"SMA_8arrSlope.[latest] = {stock.SMA_8arrSlope[-1]}")
        print(f"SMA_5arrSlope.[latest] = {stock.SMA_5arrSlope[-1]}")
