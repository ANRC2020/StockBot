    ...
    if(stock.bought == True and stock.sold == False):
        if(stock.close[-1] > stock.close[-2]):
            ...
            for order in open_orders:
                if(order.symbol == stock.Ticker):
                    if(order.side == "sell" and order.type == "limit"):
                        #cancel old order
                        api.cancel_order(order)
                        #create a new order
                        api.submit_order(symbol=stock.Ticker, qty = str(stock.quantity), side = "sell", type = "trailing_stock", trail_price = str(stock.close[-2]), time_in_force = 'day')
                        break
...
def buy(stock):
    ...
    api.submit_order(symbol=stock.Ticker, qty = str(stock.quantity), side = "buy", type = "market", time_in_force ='day')
    api.submit_order(symbol=stock.Ticker, qty = str(stock.quantity), side = "sell", type = "trailing_stock", trail_price = str(stock.close[-2]), time_in_force = 'day')
...
key = 'PKLKGFUVKHD8ECU4ACXD'
sec = '7Ab9NxD2mXD8sMhheF5vVmaGQ1yUd5ktFzTXSuje'
...
while(True):
    ...
    if check_time.hour >= 6 and check_time.hour <= 13: #check_time.hour >= 0:
        start_time = time.time()

        print("Beginning Trades!\n")

        while(check_time.hour <= 13): #check_time.hour <= 11
            end_time = time.time()
