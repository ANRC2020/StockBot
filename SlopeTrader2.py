...
global iterations #iterations * timestep = total time run
iterations = 12
...
def percent_change_update(stock, x):
    ...
    stock.changes.append((float(data.iloc[1]['Close'] - float(data.iloc[0]['Close']))/float(data.iloc[0]['Close'])) * 100)
    # stock.changes.append(((close1 - close0)/close0)*100) #Difference between closing values over time difference
    stock.limit = float(data.iloc[1]['Close']) * 1.00
    
    #Update the EMA
    stock.prev_EMA = stock.curr_EMA
    stock.curr_EMA = float(data.iloc[1]['Close'])*(2/(counter+1)) + stock.curr_EMA*(1 - (2/(counter + 1)))
                    ...
                    if(-1*tradingbound <= stocks_list[i].changes[-1] and stocks_list[i].changes[-1] <= tradingbound):
                        ...
                        if(stocks_list[i].bought == True and stocks_list[i].sold == False):
                            if(stocks_list[i].bought_price <= stocks_list[i].limit):
                                ...
                                continue
                            # if(stocks_list[i].EMA_Slope < -.1):
                            #     sell(stocks_list[i])
                            #     print(f"Made a profit of: {stocks_list[i].profit[-1]}")
                            #     continue
                            # # Hard-sell if slope drops too quickly
                            # if(stocks_list[i].changes[-1] <= HSbound):
                            #     sell(stocks_list[i])
                            #     print(f"Made a profit of: {stocks_list[i].profit[-1]}")
                        else:
