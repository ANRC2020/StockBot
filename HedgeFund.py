...
global symbol
# symbol = full_list[:5]
symbol = ['SWBI','ASPN','EVC','PRTA','BEAM','LE','SAVA','ALEC','OSG','NTLA']
...
global newBuyOrderId
newBuyOrderId = 1

global newStopLossId
newStopLossId = -1
...
def update(stocks_list,i):    
    ...
    print("Comming up on Stop-Loss Update")

    if(stock.bought == True and stock.sold == False):
            input("Correctly identified stock was purchased")
            ...
            for order in open_orders:
                print(f"order.client_order_id = {order.client_order_id} and str(stock.stoplossid) = {str(stock.stoplossid)}")
                if(order.client_order_id == str(stock.stoplossid)):
                    #cancel old order
                    print("Found Old Stop-Loss Order")
                    if(stock.close[-1] > stock.close[-2]):
                        api.cancel_order(order.id)

                        time.sleep(10)

                        stock.stoplossid = newStopLossId
                        newStopLossId -= 1
                        #create a new order
...
def buy(stock):
    ...
    stock.buyorderid = newBuyOrderId
    newBuyOrderId += 1
    api.submit_order(symbol=stock.Ticker, qty = str(stock.quantity), side = "buy", type = "market", time_in_force ='day', client_order_id = str(stock.buyorderid))

    time.sleep(10)

    # closed_orders = api.alpaca.list_orders(status="closed")
    # for order in closed_orders:
    #     if order.id == str(stock.buyorderid):
    stock.stoplossid = newStopLossId
    newStopLossId -= 1
    api.submit_order(symbol=stock.Ticker, qty = str(stock.quantity), side = "sell", type = "stop", stop_price = str(stock.close[-2]), time_in_force = 'day', client_order_id = str(stock.stoplossid))
    # newStopLossId -= 1
    # stock.stoplossid = stoplossorder.id
    ...
    for order in orders:
        if(order.client_order_id == str(stock.stoplossid)):
            if order.status == 'open':
                ...
                print(f"order.filled_avg_price = {order.filled_avg_price}")
                stock.stoplosslimit = float(order.filled_avg_price)
                if(stock.stoplosslimit != 0):
                        stock.capital = stock.capital + (stock.stoplosslimit * stock.quantity)
                        stock.profit_per_trade = stock.stoplosslimit - stock.profit_per_trade
...
key = 'PKPZAOE45P7RSM7ZOIE0'
sec = 'GymfSX8X6neO8YE7C6YYb2MopubAwcdsojEUfBQy'
