...
def startshort(stock):
    global curr_time
    print("Triggered Start Short Command\n")

    stock.quantity = int(stock.capital/stock.limit)
...
def endshort(stock):
    print("Triggered End Short Command\n")
    # order = api.submit_order(symbol=stock.Ticker,
    #                         qty = str(stock.quantity),
    #                         side ="sell",
    #                         type = "market",
    ...
    if(stock.stoplosslimit != 0):
        stock.capital = stock.capital + ((stock.bought_price - stock.stoplosslimit) * stock.quantity)
        stock.profit_per_trade = stock.profit_per_trade - stock.stoplosslimit
    else:
        stock.capital = stock.capital + ((stock.bought_price - stock.limit) * stock.quantity)
        stock.profit_per_trade = stock.profit_per_trade - stock.limit

    stock.profit.append(stock.profit_per_trade * stock.quantity)
    stock.profit_per_trade = 0
    print(f"Bought back {stock.Ticker} and made a profit of {stock.profit[-1]}")
            ...
            while(counter < iterations): # set to # of data entires - 2
                ...
                for i in range(stocks):
                    ...
                    if(IP == 0):
                        ...
                        # if(stocks_list[i].RSILong[-1] > 50):

                            if(stocks_list[i].hard_sell == False):

                                if(stocks_list[i].changes[-1] < 0):

                                    if((stocks_list[i].PSARMatrix[0][7] == 0 and stocks_list[i].PSARMatrixlong[0][7] == 0) or (stocks_list[i].MACD_val[-1] < stocks_list[i].MACD9_arr[-1])):

                                        if(stocks_list[i].bought == False and stocks_list[i].sold == True):
                                            if(stocks_list[i].EMA_Slope < 0):
                                                startshort(stocks_list[i])

                                else:

                                    if((stocks_list[i].PSARMatrix[0][7] == 1 or stocks_list[i].PSARMatrixlong[0][7] == 1) or (stocks_list[i].EMA_Slope > 0)):

                                        if(stocks_list[i].bought == True and stocks_list[i].sold == False):

                                            endshort(stocks_list[i])
