...
def setstocks(arr):
    ...
    try:
        stock = main()
    # except (RuntimeError, TypeError, NameError, ValueError, IndexError, ZeroDivisionError, json.decoder.JSONDecodeError):
    except Exception as ex:
        print(f"Ran into {ex} on {symbol[-1]}")
        # input()
        return []
    return [stock.Ticker,((stock.close[-1] - stock.close[IP] )/stock.close[IP])*100,(((netsectionval - netcapital)/netcapital)*100)]
...
def main():
    ...
    print(f"Number JSON error: {jsonerr}")

    update_mass_stocks(stocks_list[0],IP, netsectionval, netcapital)
